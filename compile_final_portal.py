            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-lg font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                        ...
                        <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800">108 Questions</span>
                    </h1>
                    <p class="text-xs text-slate-500 dark:text-slate-400">Plain-Spoken 5-7 Word Executive Interview Cadence</p>
                </div>
            </div>
...
html_content += f"""
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <!-- 9 Modules and 108 Plain-Spoken Questions -->
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800 mb-6 gap-2">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="text-xs font-semibold text-slate-500 dark:text-slate-400">12 Plain-Spoken Answers</span>
                        </div>
    ...
    for q in mod['questions']:
        ...
        html_content += f"""
                    ...
                    <article id="card-{q['id']}" class="question-card bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 p-6 shadow-sm hover:shadow-md transition-all">
                        ...
                        <!-- 3-4 Bullets, Strictly 5-7 Words Plain Spoken Structure -->
                        <div class="mt-5">
                            <div class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">Plain-Spoken Cadence (Tool → Action → Metric):</div>
...
html_content += f"""
    ...
    <div id="flashcardModal" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm hidden flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-2xl w-full p-6 sm:p-8 flex flex-col relative max-h-[90vh]">
            ...
            <div id="flashcardBody" onclick="flipSpeedCard()" class="my-6 p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 cursor-pointer min-h-[220px] flex flex-col justify-center transition-all hover:bg-slate-100/70 dark:hover:bg-slate-800 select-none">
                <div id="flashcardSidePrompt" class="text-[11px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-2">QUESTION (TAP / SPACE TO REVEAL 5-7 WORD ANSWER)</div>
    ...
    <script>
        ...
        function startBreathTimer(btn) {{
            ...
            const interval = setInterval(() => {{
                ...
                if (timeLeft > 0) {{
                    ...
                    playChime();
                    label.innerText = `Speak in 5-7 words!`;
                    setTimeout(() => {{
        ...
        function updateSpeedCard() {{
            ...
            if (!isCardFlipped) {{
                prompt.innerText = "QUESTION (TAP / SPACE TO REVEAL 5-7 WORD ANSWER)";
                ...
                prompt.innerText = "5-7 WORD PLAIN-SPOKEN ANSWER";
...
print(f"Successfully compiled {output_path} with 108 plain-spoken questions!")
